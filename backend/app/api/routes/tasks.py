...
def cancel_task(task_id: str, db: Session = Depends(get_db)):
    ...
    if task.status in {"succeeded", "failed"}:
        raise HTTPException(409, detail={"code": "TASK_CONFLICT", "message": "已结束任务不能取消"})
...
def retry_task(task_id: str, background: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    if old.status not in {"failed", "cancelled"}:
        raise HTTPException(409, detail={"code": "TASK_CONFLICT", "message": "只有失败或取消任务可以重试"})
    task = AITask(
        project_id=old.project_id,
        type=old.type,
        trace_id=old.trace_id,
        idempotency_key=f"{old.idempotency_key}-retry-{old.updated_at.timestamp()}",
